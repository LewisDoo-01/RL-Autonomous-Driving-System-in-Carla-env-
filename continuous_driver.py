...
def runner():
    ...
    update_timestep = 2048  # Số bước tối thiểu để PPO học
    ...
    try:
        ...
        if train:
            ...
            while timestep < total_timesteps:
                ...
                if observation is None:
                    print("ERROR: env.reset() failed to return an observation.")
                    print("Most likely the CARLA server is dirty. Please restart CARLA.")
                ...
                for t in range(args.episode_length):
                    ...
                    # cho phép hiện reward sang môi trường
                    env.set_hud_info(episode=episode, reward=current_ep_reward) 
                ...
                if time_since_last_update >= update_timestep:
                    
                    print(f"\n--- Đang cập nhật mô hình tại bước {timestep} ---")
                    agent.learn()
                    agent.chkpt_save()
                    
                    # Logic lưu file .pickle (giữ nguyên từ code cũ)
                    chkt_file_nums = len(next(os.walk(f'checkpoints/PPO/{town}'))[2])
                    if chkt_file_nums != 0:
                        chkt_file_nums -=1
                    chkpt_file = f'checkpoints/PPO/{town}/checkpoint_ppo_'+str(chkt_file_nums)+'.pickle'
                    
                    pickle_dir = os.path.dirname(chkpt_file)
                    if not os.path.exists(pickle_dir):
                        os.makedirs(pickle_dir)
                    
                    data_obj = {'cumulative_score': cumulative_score, 'episode': episode, 'timestep': timestep, 'action_std_init': action_std_init, 'time_since_last_update': time_since_last_update}
        ...
        import traceback
        logging.error(f"An unexpected error occured in the main loop: {e}")
        traceback.print_exc()
        sys.exit(1)
