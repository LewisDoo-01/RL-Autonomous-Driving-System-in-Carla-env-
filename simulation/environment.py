...
class CarlaEnvironment():
    ...
    def __init__(self, client, world, town, checkpoint_frequency=100, continuous_action=True) -> None:
        ...
        self.font = pygame.font.Font(None, 28) # Font mặc định, cỡ 28
        self.hud_episode = 0
        self.hud_reward = 0.0
        self.hud_speed = 0.0
    ...
    def reset(self):
        ...
        try:
            ...
            if self.display_on:
                self.env_camera_obj = CameraSensorEnv(self.vehicle, self)
            ...
            if self.fresh_start:
                ...
                for x in range(self.total_distance):
                    ...
                    # 1. Lấy danh sách waypoint tiếp theo CHỈ MỘT LẦN
                    next_waypoints_list = current_waypoint.next(1.0)

                    # 2. KIỂM TRA ĐƯỜNG CỤT (ÁP DỤNG CHO TẤT CẢ CÁC TOWN)
                    if not next_waypoints_list:
                        # Nếu là đường cụt, dừng tạo route và thoát khỏi vòng lặp
                        print(f"Warning: Route generation stopped early at step {x} due to a dead end.")
                        break 
                    
                    # 3. Nếu không có đường cụt, tiếp tục logic của bạn
                    if self.town == "Town07":
                        if x < 650:
                            next_waypoint = next_waypoints_list[0]
                        else:
                            # Đảm bảo có lựa chọn [-1] (thường là ngã rẽ)
                            next_waypoint = next_waypoints_list[-1] if len(next_waypoints_list) > 1 else next_waypoints_list[0]
                    elif self.town == "Town02":
                        if x < 650:
                            next_waypoint = next_waypoints_list[-1] if len(next_waypoints_list) > 1 else next_waypoints_list[0]
                        else:
                        ...
                        # Dành cho các town ngẫu nhiên (Town01, v.v.)
                        next_waypoint = next_waypoints_list[0]
...
# ----------------------------------------------------------------
    ...
    def set_hud_info(self, episode,  reward):
        self.hud_episode = episode
        self.hud_reward = reward
