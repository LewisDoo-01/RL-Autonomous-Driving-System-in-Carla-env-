...
class CameraSensorEnv:
    ...
    def _get_third_person_camera(weak_self, image):
        ...
        hud_episode = self.parent_env.hud_episode
        hud_reward = self.parent_env.hud_reward
        hud_speed = self.parent_env.hud_speed
        font = self.parent_env.font

        text_color = (255, 255, 255) # white

        # render text
        text_episode = font.render(f"Episode: {hud_episode}", True, text_color)
        text_reward = font.render(f"Reward: {hud_reward:.2f}", True, text_color)
        text_speed = font.render(f"Speed: {hud_speed:.1f} k/m", True, text_color)
        
        #display text
        self.display.blit(text_episode, (10, 10))
        self.display.blit(text_reward, (10, 40))
        self.display.blit(text_speed, (10, 70))
