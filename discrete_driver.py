    ...
    try:
        ...
        if exp_name == 'ddqn' and checkpoint_load:
            while agent.replay_buffer.counter < agent.replay_buffer.buffer_size:
                ...
                if observation is None:
                    print("ERROR: env.reset() failed. Khởi động lại CARLA và thử lại.")
                    break # Thoát vòng lặp training
        ...
        if args.train:
            ...
            for step in range(epoch+1, EPISODES+1):
                ...
                observation = env.reset()
                if observation is None:
                    print("ERROR: env.reset() failed. Please check CARLA server status and restart if needed.")
                ...
                while not done:
                    ...
                    # giả sử "step" là biến đếm episode
                    env.set_hud_info(episode=step, reward=current_ep_reward)
